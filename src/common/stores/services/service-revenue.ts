...
type Action = {
    ...
    setSumServiceCustomerPaid: (
        service: State["sumServiceCustomerPaid"],
    ) => void;
...
function useFetchServicesRevenue() {
    ...
    let setServicesRevenue = useServiceStore(
        (state) => state.setServicesRevenue,
    );
    ...
    let setSumServiceDebit = useServiceStore(
        (state) => state.setSumServiceDebit,
    );
